...
class PdfComposer:
    ...
    def paragraph_bold(self, text: str, style: str = "PTBody") -> Paragraph:
        return Paragraph(f"<b>{self._escape(text)}</b>", self.styles[style])
    ...
    def chart_image(self, png_bytes: bytes, *, width_mm: float = 160, height_mm: float | None = None) -> Image:
        buf = io.BytesIO(png_bytes)
        if height_mm is None:
            iw, ih = ImageReader(buf).getSize()
            height_mm = width_mm * (ih / iw) if iw else 90.0
            buf.seek(0)
        return Image(
            buf,
            width=width_mm * mm,
    ...
    def qa_table(
        ...
        *,
        ...
        num_col = num_col_mm * mm
        answer_col = answer_col_mm * mm
        question_col = max(self._page_width - num_col - answer_col, 40 * mm)
        col_widths = [num_col, question_col, answer_col]
