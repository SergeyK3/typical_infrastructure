    ...
    *,
    ...
    if sessions_by_test:
        ...
        for test_id in sorted(sessions_by_test.keys()):
            preview = _report_text(sessions_by_test[test_id]).split("\n")[0][:100]
            summaries.append(f"{test_id.upper()}: {preview or 'завершён'}")
...
def _general_summary_story(
    ...
    *,
    ...
    if ai_text:
        ...
        for test_id, doc in sorted(sessions_by_test.items()):
            preview = _report_text(doc).split("\n")[0][:120]
            lines.append(f"• {test_id.upper()}: {preview or 'данные доступны'}")
        body = "\n".join(lines)
    return [
