...
def get_account(account_id: str, db: Session = Depends(get_db)) -> AccountWithRolesOut:
...
def create_account(payload: AccountCreate, db: Session = Depends(get_db)) -> AccountWithRolesOut:
...
def patch_account(account_id: str, payload: AccountPatch, db: Session = Depends(get_db)) -> AccountWithRolesOut:
    obj = db.get(Account, account_id)
    if not obj:
        raise HTTPException(status_code=404, detail="account_not_found")
...
def reset_password(account_id: str, db: Session = Depends(get_db)) -> AccountOut:
    obj = db.get(Account, account_id)
    if not obj:
        raise HTTPException(status_code=404, detail="account_not_found")
...
def bulk_create_accounts(payload: AccountBulkCreateRequest, db: Session = Depends(get_db)) -> AccountBulkCreateResult:
